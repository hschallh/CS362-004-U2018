...
int adventuredraw(int currentPlayer, int drawntreasure, int z, int temphand[], int cardDrawn, struct gameState* state)
{
...
int smithydraw(int currentPlayer, int handPos, struct gameState* state)
{
    int i;
    //+3 Cards
    for (i = 0; i < 3; i++) {
...
int villagedraw(int currentPlayer, int handPos, struct gameState* state)
{
    //+1 Card
    drawCard(currentPlayer, state);

    //+2 Actions
    state->numActions = state->numActions + 3; //bug inserted village draw allows +3 actions instead of 2

    //discard played card from hand
    discardCard(handPos, currentPlayer, state, 0);
    return 0;
}
int stewarddraw(int choice1, int choice2, int choice3, int currentPlayer, int handPos, struct gameState* state)
{
    if (choice1 == 1) {
        //+2 cards
        drawCard(currentPlayer, state);
        drawCard(currentPlayer, state);
    } else if (choice2 == 2) //bug inserted here--choice1 switched to choice2
    {
        //+2 coins
...
int councildraw(int currentPlayer, int handPos, struct gameState* state)
{
    int i = 0;
    //+4 Cards
    for (i = 0; i < 4; i++) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState* state, int handPos, int* bonus)
{
    ...
    switch (card) {
    case adventurer:
        adventuredraw(currentPlayer, drawntreasure, z, temphand, cardDrawn, state);
        return 0;

    case council_room:
        councildraw(currentPlayer, handPos, state);
        return 0;
    ...
    case smithy:
        smithydraw(currentPlayer, handPos, state);
        return 0;

    case village:
        villagedraw(currentPlayer, handPos, state);
        return 0;
    ...
    case steward:
        stewarddraw(choice1, choice2, choice3, currentPlayer, handPos, state);
        return 0;
