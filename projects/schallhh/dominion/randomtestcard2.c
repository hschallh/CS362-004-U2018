...
int main() {
  ...
  for (j = 0; j < TRIES; j++) {
    ...
    G.deckCount[p] = rand() % (MAX_DECK - 1) + 1;
    ...
    testAssert(G.numActions == prevG.numActions + 2,
               "2 actions gained from play");
    testAssert(
        G.handCount[p] == prevG.handCount[p],
        "Hand increased by 0 (+1 for drawn card, -1 for discard village");
